...
public class Board2Controller extends HttpServlet {
	...
	protected synchronized void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if("load".equals(action)) {
			...
			try {
				list = boardService.loadNextSequence(2, userDto.getRecentStone());
			} catch (SQLException e) {
				e.printStackTrace();
			}
			if(list!=null) {
			...
			List<ChatDto> chats = boardService.loadNextChat(2, userDto.getRecentChat());
			if(chats.size()!=0) {
			...
			System.out.println("2번방 "+no+"번 플레이어 승리");
