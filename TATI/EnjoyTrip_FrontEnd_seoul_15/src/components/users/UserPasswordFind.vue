...
const { userRegist, userPasswordFind } = store;
const user = ref({
	userId: "",
	userName: "",
	userPwd: "",
...
const findPassword = async () => {
	if (user.userId || user.userName || user.emailId || user.emailDomain) {
		alert("빈칸없이 입력 해주세요");
	} else {
		await userPasswordFind(user.value);
	}
};
</script>
		...
		<form method="post" action="#">
			<div class="row uniform">
				...
				<div class="10u$">
					<ul class="actions">
						<li>
							<button type="button" @click="findPassword">Find</button>
						</li>
